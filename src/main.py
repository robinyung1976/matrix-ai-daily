...
def generate_nanjing():
    system = (
        ...
        "请联网搜索南京本地信息，用中文撰写本地专刊，Markdown格式，结构固定为：\n"
        "# 矩阵AI日报·南京 | 本地专刊\n"
        "## 本地政策速递\n"
        ...
        "搜索时不要只盯着'当天发布'：凡是发布时间在近7天内、且活动/报名截止时间仍在今天之后（未结束、可报名）的信息，"
        "都必须纳入对应栏目，并在条目中标注'报名截止时间'；某栏目确实一条可报名或未结束的都找不到，才写'今日暂无'，严禁编造。"
    )
    return call_dashscope(
        system,
        f"今天是{TODAY}。请联网搜索南京本地近7天内发布、以及仍在报名期/进行中的OPC/AI相关政策、活动与培训信息"
        f"（重点是仍在有效期内的培训与活动，如南京人社、人社局、工信局、科技局等发布的培训/训练营/公开课），生成《矩阵AI日报·南京》专刊。",
    )
